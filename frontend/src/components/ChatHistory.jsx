...
/**
 * Chat History Component for ODRL Demo
 * Stores and displays previous policy processing sessions
 * 
 * @param {Object} props
 * @param {Array} props.history - Array of history items
 * @param {function} props.onLoadHistory - Callback when user loads a history item
...
export const ChatHistory = ({ 
  ...
  const exportHistory = () => {
    ...
    const exportFileDefaultName = `odrl-history-${Date.now()}.json`;
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed left-6 bottom-6 z-40 flex items-center gap-2 px-4 py-3 rounded-lg shadow-lg transition-all ${
          darkMode 
      ...
      {/* History Panel */}
      {isOpen && (
        <div className={`fixed left-6 bottom-24 z-40 w-96 max-h-[600px] rounded-lg shadow-2xl overflow-hidden ${
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`p-2 rounded transition ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-200'}`}
                ...
                <button
                  onClick={() => {
                    if (window.confirm(`Clear all ${history.length} history items?`)) {
                      onClearHistory();
                      setIsOpen(false);
                    }
                  }}
...
const HistoryItem = ({ item, darkMode, isExpanded, onToggleExpand, onLoad }) => {
  ...
  return (
    <div className={`p-4 hover:bg-opacity-50 transition cursor-pointer ${
      ...
      <div onClick={onToggleExpand}>
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            <div className={`text-sm font-medium mb-1 line-clamp-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              {item.inputText?.substring(0, 80) || 'Untitled policy'}
          ...
          <div className={`ml-2 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
      ...
      {isExpanded && (
        <div className={`mt-3 pt-3 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          <div className="mb-3">
            ...
            <div className={`text-xs p-2 rounded ${darkMode ? 'bg-gray-900 text-gray-300' : 'bg-gray-50 text-gray-700'} max-h-24 overflow-y-auto`}>
              {item.inputText}
            </div>
          </div>
          ...
          {item.completedStages && (
            <div className="mb-3">
              ...
              <div className="flex gap-2 flex-wrap">
                {item.completedStages.map(stage => (
                  <span
                    key={stage}
                    className="text-xs px-2 py-1 rounded bg-green-500 bg-opacity-20 text-green-600 dark:text-green-400"
                  >
                    {stage}
                  </span>
                ))}
